...
|-------|------------|---------|
| Rectangle | `<sample>_common_svs.csv` | Span of each SV within a chromosome |
| Scatter   | `<sample>_common_svs.csv` | Ratio of SVTYPEs per chromosome |
...
If you want a "high-confidence only" view, filter the common-SV CSV by
...
2. Add a **rectangle** track → load `<sample>_common_svs.csv`, map
...
```python
import pandas as pd
df = pd.read_csv("results/<sample>/<sample>_common_svs.csv")
